    ...
    return (
        <section className="bg-slate-800 bg-cover py-6 text-gray-900">
            <h2 className="text-3xl md:text-4xl font-bold text-center mb-10 text-white">Projects</h2>
            
            <div className="max-w-6xl mx-auto grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-6 px-4">
                {projects.map((proj) => (
                    <div key={proj.title} className="bg-white rounded-lg shadow-lg hover:scale-105 transition">
                        <img src={proj.gif} alt={proj.title} className="rounded-t-lg w-full h-38 md:h-48 object-cover mb-2" />
                        
                        <div className="p-4 pt-2">
                            <h3 className="font-semibold text-lg md:text-xl">{proj.title}</h3>
                            <p className="text-sm text-gray-500">{proj.category}</p>
                            ...
                            <div className="flex flex-wrap gap-2 mt-4">
                                {proj.tech.map((tech, i) => (
                                    <span 
                                        key={i} 
                                        className="bg-cyan-400 px-3 py-1 rounded-full text-sm font-bold shadow-md shadow-blue-600"
                                    >
                                        {tech}
                                    </span>
                                ))}
